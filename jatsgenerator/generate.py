...
class ArticleXML(object):
    ...
    def __init__(self, poa_article, jats_config):
        """
        set the root node
        ...
        comment = Comment('generated by ' + str(self.jats_config.get('generator'))
                          + ' at ' + generated + ' from version ' + last_commit)
...
def output_xml_for_article(article, article_id, config_section="elife"):
    raw_config = config[config_section]
    jats_config = parse_raw_config(raw_config)
    try:
        article_xml = ArticleXML(article, jats_config)
...
def build_xml_from_csv(article_id, config_section="elife"):
    # override settings
    data.CSV_PATH = 'tests/test_data/'
    article, error_count, error_messages = parse.build_article(article_id)
    if article:
        return output_xml_for_article(article, article_id, config_section)
    else:
